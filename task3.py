...
def benchmark(func: Callable, text_: str, pattern_: str):
    ...
    times = timeit.repeat(stmt=stmt, setup=setup_code, globals={'text': text_, 'pattern': pattern_}, repeat=5, number=10)
    return min(times)

if __name__ == '__main__':
    filenames = {
        'стаття 1.txt': {'real': "Жадібний алгоритм", 'fake': "GoIT"},
        'стаття 2.txt': {'real': "Зв’язний список", 'fake': "GoIT"}
    }
    ...
    for result in results:
        print(f"{result[0]:<20} | {result[1]:<20} | {result[2]:<15} | {result[3]:<10.6f}")
